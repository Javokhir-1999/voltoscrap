...
async def get_channel_messages(search: dto.SearchDTO):
    print(search.words, search.telegram_channels, search.telegram_limit)
    client = TelegramClient('bot', api_id, api_hash)

    await client.start()
    # Find the channel by its username
    for tg_channel in search.telegram_channels.split(','):
        try:
            channel = await client.get_entity(tg_channel)
        except:
            print('no channel by this name:', tg_channel)
            continue
